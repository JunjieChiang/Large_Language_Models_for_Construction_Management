...
def call_with_messages(llm, query, context):
    ...
    {query}
    属性信息以JSON数据形式存放，你需要从中提取用户查询的属性值。其中与用户查询相关的属性信息如下：
    {context}
    ...
    举个例子：
    用户查询问题：该构件的体积是多少？
    你应该返回：0.000265166628679756（如果JSON对象中存在该属性）。
...
def main():
    ...
    relevant_docs = info_retrieval.main(args.query, args.data_path, args.index_path, args.embedding_model,
                                        args.vectorised_KB, args.json_name_path, args.top_k)
